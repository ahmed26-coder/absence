...
import { updatePaymentRequestStatus } from "@/lib/supabase-storage"
import Image from "next/image"

type JoinedStudent = { full_name: string | null; avatar_url: string | null }
type JoinedDebt = { id: string; name: string; amount_owed: number; amount_paid: number }
...
interface PaymentRequestRow {
    ...
    created_at: string
    // Supabase types to-one joins loosely (object or array); normalized per-row below.
    student: JoinedStudent | JoinedStudent[] | null
    debt: JoinedDebt | JoinedDebt[] | null
}
            ...
            {requests.length === 0 ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {requests.map((request) => {
                        ...
                        const student = Array.isArray(request.student) ? request.student[0] : request.student;
                                ...
                                <CardHeader className="bg-gray-50/50 pb-4">
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <CardTitle className="text-base font-bold">{student?.full_name || "طالب غير معروف"}</CardTitle>
