...
interface AddStudentsToCourseModalProps {
  ...
  onAddStudents: (studentIds: string[]) => Promise<boolean>
}

export const AddStudentsToCourseModal: React.FC<AddStudentsToCourseModalProps> = ({
  isOpen,
  onClose,
  courseId,
  courseName,
  ...
  const filteredStudents = useMemo(() => {
    ...
    return nonEnrolledStudents.filter((s) => s.name.toLowerCase().includes(term))
  }, [nonEnrolledStudents, searchTerm])
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true)
    try {
      const success = await onAddStudents(Array.from(selectedIds))
      if (success) {
        pushToast(`تمت إضافة ${selectedIds.size} طالب(ة) بنجاح`, "success")
        setSelectedIds(new Set())
        setSearchTerm("")
        onClose()
      } else {
        pushToast("تعذر إضافة الطلاب", "error")
      }
    } catch (error) {
      console.error("Error adding students:", error)
      pushToast("حدث خطأ أثناء إضافة الطلاب", "error")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <Dialog
      ...
      title="إضافة طالب إلى الدورة"
      description={`أضف طلاباً إلى دورة ${courseName}`}
    >
      <div className="w-full max-w-md space-y-4">
        ...
        {filteredStudents.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-sm font-semibold">الطلاب المتاحين</label>
              <button
                type="button"
        ...
        {filteredStudents.length === 0 && !searchTerm && (
          <div className="rounded-lg border border-dashed border-border/50 bg-muted/30 p-6 text-center">
            <p className="text-sm text-muted-foreground">جميع الطلاب الموجودين مسجلون بالفعل في هذه الدورة.</p>
          </div>
        )}
